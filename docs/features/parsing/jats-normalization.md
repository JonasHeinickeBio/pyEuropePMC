...
| `doi`, `pmcid`, `pmid` | `str` | The first `<article-id>` of each type |
| `article_type` | `str` | The root element's `article-type` attribute, lower-cased |
| `authors` | `list[dict]` | Every author with a `<name>`: a `<contrib contrib-type="author">`, or an untyped `<contrib>` in a `<contrib-group content-type="author">`. Each is `{"name": "Given Surname"}`, plus `"orcid"` when the contributor has an ORCID `<contrib-id>` |
| `journal` | `str` | The first `<journal-title>` |
| `license` | `str` | Text of the first `<license-p>`, or the `license-type` attribute |

Everything except `article_type` is read from the article's own `<front>`, so the DOI and contributors of a peer-review `<sub-article>` are not the article's.
...
|---|---|---|
| 1 | always | `bytes` input is decoded with the encoding its byte order mark gives, else the `encoding` of its XML declaration, else UTF-8. A declared encoding Python does not know is read as UTF-8 |
| 2 | always | In the raw text, named entities from a built-in table (`&alpha;` becomes `α`, `&nbsp;` a space, `&ndash;` a hyphen) are replaced by their characters. Numeric character references (`&#x2013;`, `&#x0003c;`) and `&amp;`, `&lt;`, `&gt;`, `&quot;` and `&apos;` are left to the XML parser |
...
| 5 | `strip_display_markup`, `flatten_xrefs` | In the body. With `strip_display_markup`, `bold`, `italic`, `underline`, `strike`, `sc`, `sub`, `sup`, `monospace`, `roman`, `sans-serif`, `styled-content`, `named-content` and `overline` are removed, and their text and child elements stay where they were. With `flatten_xrefs`, `xref`, `ext-link`, `uri` and `email` are replaced by their text, or by their `rid` or `href` when they have none |
...
| 7 | `drop_mathml` | MathML `math` elements are replaced by their text, so the MathML for x² + y becomes `x2+y`. In an `<alternatives>` that also holds a `<tex-math>`, the TeX is kept and the MathML removed without its text |
...
| `strip_display_markup` | `bool` | `True` | Step 5, display markup |
| `flatten_xrefs` | `bool` | `True` | Step 5, cross-references and links |
...
The list is in document order, each section followed by its own subsections:
...
{'section_type': 'intro', 'section_title': 'Introduction'}
```
...
| `batch INPUT_DIR OUTPUT_DIR` | One file per `*.xml`: `NAME.txt`, `NAME.sections.json` or `NAME.bioc.json`, then a summary line. The exit status is 1 when any file failed | `--output-format text\|sections\|bioc` (default `text`), `--recursive/--no-recursive` (default `--no-recursive`), `--no-entities`, `--no-markup`, `--no-sections` |
...
- `--no-markup` sets only `strip_display_markup=False`. Display markup stays in the tree, so `text` prints the pieces of text around each `<bold>` or `<italic>` on lines of their own; cross-references are still flattened.

The commands read files as bytes, so a file is decoded the way its XML declaration says. `text` and `bioc` print the document as it is, square brackets included.
