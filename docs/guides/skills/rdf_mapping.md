...
- `conf/rdf_map.yml` is also the source of the RML file `conf/rml_mappings.ttl` used by `RMLRDFizer`. After editing the YAML, regenerate it with `make sync-rdf` or `python examples/scripts/sync_rdf_mappings.py`.
