...
class PaperEntity(ScholarlyWorkEntity):
    ...
    external_ids: str | None = Field(
        default=None,
        description="""External identifiers from various sources""",
        json_schema_extra={
            "linkml_meta": {"domain_of": ["PaperEntity"], "slot_uri": "pyeuropepmc:externalIds"}
        },
    )
    ...
    authors: list[AuthorEntity] | None = Field(
        ...
        json_schema_extra={
            "linkml_meta": {"domain_of": ["PaperEntity"], "slot_uri": "dcterms:creator"}
        },
    )
    ...
    journal: JournalEntity | None = Field(
        ...
        json_schema_extra={
            "linkml_meta": {"domain_of": ["PaperEntity"], "slot_uri": "bibo:journal"}
        },
    )
    ...
    @classmethod
    def pattern_journal_issn(cls, v: Any) -> Any:
        pattern = re.compile(r"^\d{4}-\d{3}[\dX]$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
...
class CitationContextEntity(BaseEntity):
    """
    Entity representing the context of a citation within a section. Contains citation type,
...
class GrantEntity(BaseEntity):
    ...
    @classmethod
    def pattern_fundref_doi(cls, v: Any) -> Any:
        pattern = re.compile(r"^10\.\d{4,9}/[-._;()/:A-Z0-9]+$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
...
class AffiliationEntity(BaseEntity):
    """
    Entity representing an author's affiliation with an institution. Contains the relationship
    between authors and institutions with affiliation text.
    """

    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta(
        {"class_uri": "org:Membership", "from_schema": "https://w3id.org/pyeuropepmc"}
    )
