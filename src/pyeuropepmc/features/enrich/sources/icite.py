...
class ICiteClient(BaseEnrichmentClient):
    ...
    def enrich_many(
        ...
        for rec in records:
            pmid = str(rec.get("pmid", rec.get("pmid", "")))
            if pmid:
    ...
    def _normalize_result(self, raw: dict[str, Any]) -> dict[str, Any]:
        return {
            "pmid": str(raw.get("pmid", raw.get("pmid", ""))),
            "rcr": safe_float(raw.get("rcr", raw.get("rcr", 0))),
            "percentile": safe_float(raw.get("percentile", raw.get("percentile", 0))),
            "citation_count": int(raw.get("citation_count", raw.get("citationCount", 0))),
            "expected_citations": safe_float(
                raw.get("expected_citations", raw.get("expectedCitations", 0))
            ),
            "field_citation_ratio": safe_float(
                raw.get("field_citation_ratio", raw.get("fieldCitationRatio", 0))
            ),
            "nih_percentile": safe_float(raw.get("nih_percentile", raw.get("nihPercentile", 0))),
            "is_controversial": bool(
                raw.get("is_controversial", raw.get("isControversial", False))
            ),
        }
