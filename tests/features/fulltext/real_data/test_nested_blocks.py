...
def _nested(root: ET.Element, tag: str) -> list[ET.Element]:
    """``tag`` elements with a <p> somewhere above them, in document order."""
    found: list[ET.Element] = []

    def walk(node: ET.Element, in_paragraph: bool) -> None:
        for child in node:
            if in_paragraph and _local(child.tag) == tag:
                found.append(child)
            walk(child, in_paragraph or _local(child.tag) == "p")

    walk(root, _local(root.tag) == "p")
    return found
...
def _glued_cells(table: ET.Element) -> list[str]:
...
def test_the_fixtures_nest_tables_and_figures_in_paragraphs() -> None:
...
class TestStructuredBlocks:
    """get_full_text_sections_structured() on tables and figures inside paragraphs."""

    def test_each_nested_table_has_a_table_block_with_its_rows(self, doc: _Parsed) -> None:
    ...
    def test_each_nested_figure_has_a_figure_block(self, doc: _Parsed) -> None:
    ...
    def test_each_nested_block_keeps_its_caption_and_cells(self, doc: _Parsed) -> None:
    ...
    def test_nested_table_cells_are_not_run_together(self, doc: _Parsed) -> None:
    ...
    def test_title_and_abstract_are_not_labelled_body(self, doc: _Parsed) -> None:
...
class TestPlaintext:
    """to_plaintext() on tables and figures inside paragraphs."""
    ...
    def test_a_nested_figure_label_is_not_run_into_its_caption(self, doc: _Parsed) -> None:
        found = []
        for figure in _nested(doc.root, "fig"):
