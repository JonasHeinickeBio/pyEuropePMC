...
class TestZoteroClientInit:
    def test_import_error_when_not_available(self):
        with (
            patch("pyeuropepmc.features.bibliography.zotero.ZOTERO_AVAILABLE", False),
            pytest.raises(ImportError, match="pyzotero"),
        ):
            ZoteroClient(library_id="123", api_key="key")

    def test_warning_when_no_api_key(self):
        with (
            patch("pyeuropepmc.features.bibliography.zotero.ZOTERO_AVAILABLE", True),
            patch("pyeuropepmc.features.bibliography.zotero.os.getenv", return_value=""),
            patch("pyeuropepmc.features.bibliography.zotero.logger") as mock_logger,
        ):
...
class TestZoteroClientApiError:
    def test_search_raises_on_missing_credentials(self):
        with (
            patch("pyeuropepmc.features.bibliography.zotero.ZOTERO_AVAILABLE", True),
            patch("pyeuropepmc.features.bibliography.zotero._PyZotero") as MockPyZotero,
        ):
