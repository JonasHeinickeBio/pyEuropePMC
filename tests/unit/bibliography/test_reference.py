...
from pyeuropepmc.features.bibliography.reference import ReferenceResolver, detect_identifier_type, IdentifierType
...
class TestReferenceResolver:
    ...
    def test_resolve_doi(self, mock_crossref):
        mock_ref = Reference(title="Test Paper", doi="10.1234/test", authors=["Smith, J"], source="crossref")
    ...
    def test_resolve_pmid(self, mock_get_client):
        ...
        mock_client.search.return_value = {
            "results": [{"title": "PM Paper", "doi": "10.1234/b", "pmid": "12345678", "pubYear": "2020", "journalTitle": "J", "authorString": "Doe, J"}]
        }
        resolver = ReferenceResolver()
    ...
    def test_resolve_many(self):
        resolver = ReferenceResolver()
        with patch.object(resolver, "resolve_doi", return_value=Reference(title="Paper A", doi="10.1234/a", source="crossref")):
            with patch.object(resolver, "resolve_pmid", return_value=Reference(title="Paper B", pmid="99999", source="europe_pmc")):
                ids = ["10.1234/a", "99999"]
    ...
    def test_resolve_auto_detect(self):
        resolver = ReferenceResolver()
        with patch.object(resolver, "resolve_doi", return_value=Reference(doi="10.1234/x", source="crossref")) as mock_doi:
