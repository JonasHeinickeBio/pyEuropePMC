    ...
    return [
        {
            ...
            "keywordList": {"keyword": ["cancer", "immunotherapy", "checkpoint inhibitors"]},
            "meshHeadingList": {
                "meshHeading": [
        ...
        },
        {
            ...
            "meshHeadingList": {"meshHeading": [{"descriptorName": "Diabetes Mellitus"}]},
        },
        {
            "id": "11111",
            "source": "MED",
...
def test_filter_by_type(sample_papers):
    ...
    filtered = filter_pmc_papers(sample_papers, allowed_types=("Review", "Systematic Review"))
...
def test_filter_by_keywords(sample_papers):
    ...
    filtered_partial = filter_pmc_papers(
        sample_papers, required_keywords={"efficacy"}, open_access=None
    )
...
def test_filter_by_abstract_terms(sample_papers):
    ...
    filtered = filter_pmc_papers(sample_papers, required_abstract_terms={"checkpoint"})
...
def test_nofilters(sample_papers):
    ...
    filtered = filter_pmc_papers(
        sample_papers, min_citations=0, min_pub_year=2000, open_access=None
    )
...
def test_partial_abstract_matching_case_insensitive(sample_papers):
    ...
    # Use various casings
    filtered_lower = filter_pmc_papers(sample_papers, required_abstract_terms={"checkpoint"})
    filtered_upper = filter_pmc_papers(sample_papers, required_abstract_terms={"CHECKPOINT"})
    filtered_mixed = filter_pmc_papers(sample_papers, required_abstract_terms={"ChEcKpOiNt"})
...
def test_all_required_terms_must_match(sample_papers):
    ...
    filtered = filter_pmc_papers(sample_papers, required_keywords={"immunotherapy", "diabetes"})

    # Should be empty because no single paper has both
    assert len(filtered) == 0

    # But if we only require one term, we should get results
    filtered_single = filter_pmc_papers(sample_papers, required_keywords={"immunotherapy"})
...
def test__meets_type_criteria_various_types():
    ...
    # pubTypeList as list
    assert (
        _meets_type_criteria({"pubTypeList": {"pubType": ["Review", "Other"]}}, ("Review",))
        is True
    )
    assert _meets_type_criteria({"pubTypeList": {"pubType": ["Other"]}}, ("Review",)) is False
...
def test_filter_max_pub_year():
    ...
    filtered = filter_pmc_papers(
        papers, min_citations=0, min_pub_year=0, max_pub_year=2020, open_access=None
    )
    ids = [p["id"] for p in filtered]
    assert ids == ["A"]

    # same with OR variant
    filtered_or = filter_pmc_papers_or(
        papers, min_citations=0, min_pub_year=0, max_pub_year=2020, open_access=None
    )
